      ...
      <v-row>
        <v-col>
          <v-btn
            block
            color="primary"
            :disabled="!atLeastOneFilterActive()"
            @click="resetFilters()"
            >Reset Filters</v-btn
          >
        </v-col>
...
export default {
  ...
  methods: {
    atLeastOneFilterActive() {
      ...
      Object.values(this.filtersActive).forEach((activeFilter) => {
        numberOfActiveFilters += activeFilter.length;
      });
      return numberOfActiveFilters > 0;
    },
    resetFilters() {
      this.filtersActive = {};
      this.$emit("filterSelectionUpdated", this.filtersActive);
    },
  },
};
</script>
...
<style>
/* Not scoped because we have to access style properties from child component VueMultiselect */
/* Source: https://github.com/shentao/vue-multiselect/blob/master/src/Multiselect.vue */
