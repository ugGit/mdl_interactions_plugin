...
class local_moodle_ws_la_trace_exporter_external extends external_api
{
  ...
  public static function get_course_data($courseids)
  {
    ...
    $params = self::validate_parameters(
      self::get_course_data_parameters(),
      array(
        'courseids' => $courseids,
      )
    );
    ...
    $sqlAssignedRoles = "SELECT ra.id AS 'roleassignmentid', c.id AS 'courseid', ra.userid AS 'userid', r.shortname FROM {course} c 
            LEFT OUTER JOIN {context} cx ON c.id = cx.instanceid 
            LEFT OUTER JOIN {role_assignments} ra ON cx.id = ra.contextid
    ...
    $result = [];
    error_log('My variable assignedRoles is: ' . var_export($assignedRoles, true));
   ...
   * @return external_function_parameters
   */
  public static function test_fun_parameters()
  {
    return new external_function_parameters(
      array()
    );
   ...
   * @return external_description
   */
  public static function test_fun_returns()
  {
    return new external_multiple_structure(
      new external_single_structure(
        array(
